...
- **Real login/logout**: `POST /api/auth/login` checks email+password
  against a stored, bcrypt-hashed user list and returns a JWT; role-gated
  endpoints let Super Admin/Admin accounts create, list, and delete users

## Login accounts
On first boot (empty `data/users.json`), the server auto-creates three
starter accounts and prints them once to the server log — check your
Render "Logs" tab right after the first deploy to find them:

| Role | Email | Password |
|---|---|---|
| Super Admin | admin@islglobal.edu | ChangeMe123! |
| Counselor | nusrat.jahan@islglobal.edu | Counselor123! |
| Counselor | tanvir.ahmed@islglobal.edu | Counselor123! |

**Change these passwords immediately after your first login** (Admin &
Settings → Roles & Permissions → edit user, or delete and recreate the
account). New staff accounts can be added the same way — only Super
Admin/Admin roles can create or remove users.
...
4. Add the environment variables from `.env.example` under Settings → Environment
   (make sure to set a real, long random string for `JWT_SECRET` — never
